...
Here's the mechanism, and it's more mundane than "it came apart." The reset seeds the *bar* body's joints, nudging it north to its start pose exactly as a PushT reset would. But the stem is a separate body now, with its own joints, and nobody seeded those: the bar walks to its seeded position while the stem stays home, and the "T" settles as two rectangles a gap apart. It can't even be *placed* as one piece, because assembling it would mean positioning two bodies and the reset only knew about one. That settled gap is the signature to memorize: when a thing you meant to be one rigid object can't even be set down as one, the rigidity was never there. You drew a T, but you built two blocks.

What happens *during* the push is a red herring the code refuses to be fooled by. The pusher meets the loose stem first and shoves it around near the bar, so the two halves actually drift back toward 0.06 m before the run ends. That's why the rigidity verdict watches the *peak* deviation from 0.06 over the whole run, not the final gap: 0.0 for the welded T (the gap is 0.06 at every step), 0.031 m or more for the split one. Two loose pieces can wander back together by accident, but a gap that was *ever* off 0.06 was never a weld.
