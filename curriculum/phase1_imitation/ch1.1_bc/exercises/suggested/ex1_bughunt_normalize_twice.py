...
This is the chapter artifact with EXACTLY ONE conceptual bug injected: a
normalization applied somewhere it does not belong. The cruel part — train
loss and val loss are IDENTICAL to bc.py's, epoch for epoch, and the ONNX
...
Objective tested: implement supervised imitation end-to-end / diagnose a
failure no dataset metric can see.

Run:  python ex1_bughunt_normalize_twice.py --smoke --seed 0 --no-rerun
...
# This exercise sits two directories deeper than the chapter artifact.
...
parser.add_argument("--out", type=Path, default=Path("outputs/ch1.1-bc-ex1"))
...
parser.add_argument("--device", choices=("cpu", "mps"), default="cpu")  # cpu: deterministic | mps: faster, statistical repro only
...
# TorchScript is self-contained: assert_parity's CLI reloads it cross-process.
...
for episode in range(args.eval_episodes):
    ...
    while not done:
        with torch.no_grad():
            normalized_obs = 2.0 * (obs_now - obs_min) / obs_range - 1.0  # match the training distribution
            obs_batch = torch.from_numpy(normalized_obs).to(device).unsqueeze(0)  # (10,) -> (1, 10)
