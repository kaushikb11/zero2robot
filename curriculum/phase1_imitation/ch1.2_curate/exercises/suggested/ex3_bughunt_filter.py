...
This is the chapter artifact with EXACTLY ONE conceptual bug injected, in the
curation step: a mask polarity error. Every quality metric still computes and
prints; the run completes and writes metrics.json. The tell is the payoff — the
...
checks.py until the curated policy beats raw again.

Objective tested: filter bad episodes / read a data-quality result honestly.

Run:  python ex3_bughunt_filter.py --smoke --seed 0 --no-rerun
