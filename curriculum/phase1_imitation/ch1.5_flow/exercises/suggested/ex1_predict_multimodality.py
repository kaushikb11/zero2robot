...
Objective tested: the chapter's THESIS carries over from ch1.4 unchanged — a
generative model captures a multimodal target where a squared-error regressor
collapses to the average — and it holds for flow matching too. Same 2D toy, before
any robot, so the effect is measured, not asserted. The point of running it here:
the collapse is about the OBJECTIVE (sample vs average), so swapping diffusion for
flow matching does not bring it back.

THE SETUP. flow.py builds a unit-ring target: every direction around the circle is
an equally-good "mode", and the empty CENTER is the one place no data point sits.
It then trains two models on that ring with the SAME MLP width and the SAME data:
  - a flow-matching velocity net (learns to flow noise into a SAMPLE on the ring), and
  - a one-shot MSE regressor (maps noise straight to a point).
It samples 2000 points from each and reports, in metrics.json:
  - toy_flow_modes_covered / toy_regress_modes_covered  (of 8 angular sectors)
  - toy_flow_mean_radius   / toy_regress_mean_radius     (ring radius is 1.0)
...
PREDICT before you run: which row describes what the metrics will show?
  ...
  B) Flow covers all 8 modes at radius ~1 (on the ring); regression covers
     ~0 modes at radius ~0 — it lands in the empty center, the average of every
     direction, exactly where no data is.
  C) Regression covers more modes — integrating an ODE is noisy, so flow smears off
...
def config_flags() -> list[str]:
    return ["--flow_steps", str(RC["flow_steps"]), "--model_dim", str(RC["model_dim"]),
...
if __name__ == "__main__":
    ...
    with tempfile.TemporaryDirectory() as tmp:
        m = run_flow(Path(tmp) / "run")
    print(f"flow       -> modes {m['toy_flow_modes_covered']}/8  mean_radius {m['toy_flow_mean_radius']:.2f}")
    ...
    print("\nNow explain it: both models saw the same ring and have the same width. "
          ...
          "(Same answer as ch1.4 — that is the point.)")
