"""Export the trained ch2.4 reward-design policies to ONNX (tensor contract v1).

rewards.py trains the SAME from-scratch PPO under four reward *programs* and, for
the two the site demo shows, `torch.save`s the Agent state_dict to
outputs/ch2.4-rewards/{shaped_walk,height_hack}.pt:

  shaped_walk  the DENSE shaped reward — the walk that emerges from a standing start.
  height_hack  the NAIVE 'be tall' reward — reward climbs, forward distance ~0 (the
               specification-gaming demo). Same architecture; only the reward differed.

Those checkpoints are the source of truth; this is a FOLLOW-UP script (kept out of
rewards.py's readable loop, per the embed.yaml note that the ONNX export is wired
when the chapter graduates from spike to drop).
...
DETERMINISTIC MEAN — no sampling — so each exported policy is just actor_mean
wrapped as a stateless obs[1,23] -> action[1,8] nn.Module, exactly the interface
tensor contract v1 wants (see curriculum/common/export_onnx.py). action[1,8] is
the residual target the QuadrupedEnv adds to its trot prior.

Pipeline (mirrors export_walk_onnx.py, one arm per demo panel):
  train rewards.py -> export_policy(actor_mean wrapper, 23, 8) -> assert_parity ->
  copy each .onnx to site/public/models/ (git-ignored artifact; never committed).

Run with the repo venv, from the repo root:
  .venv/bin/python curriculum/phase2_reinforcement/ch2.4_rewards/export_rewards_onnx.py
Smoke (no checkpoints; random-init actors, proves the export+parity path):
  .venv/bin/python curriculum/phase2_reinforcement/ch2.4_rewards/export_rewards_onnx.py --smoke
"""
...
ARMS = {"shaped_walk": "shaped_walk.onnx", "height_hack": "height_hack.onnx"}
SITE_MODELS = REPO_ROOT / "site" / "public" / "models"


def layer_init(layer: nn.Linear, std: float = np.sqrt(2.0)) -> nn.Linear:
    """Orthogonal init — copied verbatim from rewards.py so the rebuilt Agent's
...
class Agent(nn.Module):
    """Byte-for-byte the rewards.py Agent architecture (critic + actor_mean +
    actor_logstd), redefined here because rewards.py runs argparse/training at
    import time and cannot be imported as a module. Only the parameter layout
    matters — we load the trained weights, then export actor_mean alone."""
...
class DeterministicPolicy(nn.Module):
    ...
    rewards.py's eval acts (agent.actor_mean(obs)), so the browser and the Python
    eval share one deterministic function. Contract v1: [1,23]->[1,8]."""
...
def main() -> None:
    ...
    parser.add_argument("--ckpt_dir", type=Path, default=Path("outputs/ch2.4-rewards"),
                        help="dir holding shaped_walk.pt / height_hack.pt saved by rewards.py")
    parser.add_argument("--out_dir", type=Path, default=Path("outputs/ch2.4-rewards"))
    parser.add_argument("--hidden_dim", type=int, default=64)  # rewards.py default
        ...
        if args.smoke:
            ...
            if not ckpt.is_file():
                sys.exit(f"no checkpoint at {ckpt} — train it first:\n"
                         f"  .venv/bin/python curriculum/phase2_reinforcement/ch2.4_rewards/rewards.py --seed 0 --device cpu")
