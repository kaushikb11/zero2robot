"""Export the trained ch2.5 walk actor to ONNX (tensor contract v1) for the site.

walk.py trains the Quadruped SAC policy and `torch.save`s the Actor state_dict to
outputs/ch2.5-walk/walk_actor.pt. That checkpoint is the source of truth; this is a
FOLLOW-UP script (kept out of walk.py's readable loop, per the embed.yaml note that
the ONNX export is wired when the chapter graduates from spike to drop).

The Actor is the same squashed-Gaussian policy as ch2.2: trunk(obs) feeds a
state-dependent mean and log_std. Eval (and the browser) act with the DETERMINISTIC
mean — tanh(mean), no sampling — exactly how walk.py's sample() returns its
deterministic_action. The exported policy is that tanh(mean) path wrapped as a
stateless obs[1,23] -> action[1,8] nn.Module, the interface tensor contract v1
wants (see curriculum/common/export_onnx.py). action[1,8] is the residual target
the QuadrupedEnv adds to its trot prior — the runtime feeds it straight through.

Pipeline (mirrors export_ppo_onnx.py):
  train walk.py -> export_policy(deterministic wrapper, 23, 8) -> assert_parity ->
...
Run with the repo venv, from the repo root:
  .venv/bin/python curriculum/phase2_reinforcement/ch2.5_walk/export_walk_onnx.py
Smoke (no checkpoint; random-init actor, proves the export+parity path):
  .venv/bin/python curriculum/phase2_reinforcement/ch2.5_walk/export_walk_onnx.py --smoke
"""
...
LOG_STD_MIN, LOG_STD_MAX = -5.0, 2.0  # must match walk.py

# The site fetches this path (git-ignored, provisioned like bc_policy.onnx).
SITE_MODEL = REPO_ROOT / "site" / "public" / "models" / "walk_actor.onnx"


class Actor(nn.Module):
    """Byte-for-byte the walk.py Actor (trunk + mean + log_std heads), redefined
    here because walk.py runs argparse/training at import time and cannot be
...
class DeterministicPolicy(nn.Module):
    ...
    how walk.py's eval acts (the deterministic_action sample() returns), so the
    browser and the Python eval share one function. Contract v1: [1,23]->[1,8]."""
...
def main() -> None:
    ...
    parser.add_argument("--ckpt", type=Path, default=Path("outputs/ch2.5-walk/walk_actor.pt"),
                        help="the Actor state_dict walk.py saved")
    parser.add_argument("--out", type=Path, default=Path("outputs/ch2.5-walk/walk_actor.onnx"))
    parser.add_argument("--hidden_dim", type=int, default=256)  # walk.py default
    ...
    if args.smoke:
        ...
        if not args.ckpt.is_file():
            sys.exit(f"no checkpoint at {args.ckpt} — train it first:\n"
                     f"  .venv/bin/python curriculum/phase2_reinforcement/ch2.5_walk/walk.py --seed 0 --device cpu")
