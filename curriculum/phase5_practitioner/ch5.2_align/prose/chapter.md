...
reference run from ~0.83 to ~0.38 — yet stays *above* random. (Those figures are a
provisional draft, measured at a reduced config — episodes 70, epochs 25, not the default
80/30 — so read them for their *direction*, not the exact percentages.) That "plausible but worse"
