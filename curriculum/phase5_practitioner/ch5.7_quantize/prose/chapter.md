...
`calibrate` (in the same **quantize** region above) runs a **calibration set**, a handful of held-out states, through the fp32 network
...
One caveat we skip on purpose: our scale is **symmetric** (a signed grid centered at zero), and that is a
**choice**, not the only option, that leaves range on the table for the post-**ReLU** activations feeding
layers two and three, since those are never negative and a symmetric grid spends its entire lower half on
codes that never fire; **asymmetric** (zero-point) quantization stores an integer offset so all 255 codes
cover the range actually used, and the P2 module reaches for exactly that.
