...
- **`lerobot-train`: this is your BC/ACT, or your VLA fine-tune.** `--policy.type=act` loads the full ACT, the action-chunking transformer you wrote in ch1.3, now *with* the CVAE latent that ch1.3 deliberately cut for clarity; it auto-adapts to your robot's motor count and cameras from the recorded dataset. Swap in `smolvla` or `pi0` and you are running the SmolVLA fine-tune path (the 1.8 Scale Lab, with 5.6's adapters underneath) on real demonstrations.
...
- **Plus ~$20–60 to actually record and deploy.** That BOM is arm-only (servos, printed parts, controller, wiring). Every policy in this module is visuomotor, so budget at least one USB camera, and the 12V servo option needs a power supply; neither is in the `$121.94` / `$229.88` totals.
